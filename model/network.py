...
class my_classifier(nn.Module):
    def __init__(self, input_dim, class_num, bottleneck_dim=256):
        ...
        self.bn1 = nn.BatchNorm1d(num_features=bottleneck_dim, affine=True)
        self.relu = nn.ReLU(inplace=True)
...
class my_classifier_two(nn.Module):
    def __init__(self, class_num, bottleneck_dim=256, mid_dim=128):
        super(my_classifier_two, self).__init__()
        self.fc1 = nn.Linear(bottleneck_dim, mid_dim)
        self.bn1 = nn.BatchNorm1d(num_features=mid_dim, affine=True)
        self.relu1 = nn.ReLU(inplace=True)
        self.fc2 = nn.Linear(mid_dim, class_num)
        self.bn2 = nn.BatchNorm1d(num_features=class_num, affine=True)
...
class encoder(nn.Module):
    def __init__(self, args):
        ...
        #     self.netF = ViT().cuda()
        # elif args.netF == 'mae':
        #     self.netF = vit_base_patch16().cuda()
    ...
    def load_model(self):
        modelpath = osp.join(self.args.output_dir_src, 'source_F.pt')
        self.netF.load_state_dict(torch.load(modelpath))
        # self.netF.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(modelpath).items()})
        modelpath = osp.join(self.args.output_dir_src, 'source_B.pt')
        # self.netB.load_state_dict({k.replace('module.', ''): v for k, v in torch.load(modelpath).items()})
